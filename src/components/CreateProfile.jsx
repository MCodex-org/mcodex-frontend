...
const CreateProfile = () => {
  ...
  return (
    <form onSubmit={handleSubmit} className="fieldset bg-base-200 border-base-300 rounded-box w-xs border p-4">
      <legend className="fieldset-legend">Complete your profile</legend>

      <label className="label">Username</label>
      <input onChange={(e) => setFormData({...formData, username: e.target.value})} type="text" className="input" placeholder="Username" />
      {errors.username && <p className="text-error">{errors.username}</p>}

      <label className="label">Display Name</label>
      <input onChange={(e) => setFormData({...formData, display_name: e.target.value})} type="text" className="input" placeholder="Display Name" />
      ...
      <button type="submit" disabled={loading} className="btn btn-neutral mt-4">
        {(loading && <span className="loading loading-spinner" />) || <>Create Profile</>}
      </button>
